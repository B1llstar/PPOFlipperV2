...
public class NmzDebugScript extends Script {

    private NmzDebugConfig config;
    private NmzDebugPlugin plugin;
    ...
    public boolean canStartNmz() {
        int overloadCount = Rs2Inventory.count("overload (4)");
    ...
    @Inject
    public NmzDebugScript(NmzDebugPlugin plugin, NmzDebugConfig config) {
        this.plugin = plugin;
        this.config = config;
    }
    ...
    private static final int BARREL_MAX_DOSES = 255;
    ...
    public void handleStore() {
        ...
        // Varbits are in doses; config is in 4-dose potions. Never target more than the
        // barrel can physically hold, or this would keep trying to buy potions that can't
        // be stored once the barrel is already full (or the configured amount overshoots 255).
        int overloadTargetDoses = Math.min(config.overloadPotionAmount() * 4, BARREL_MAX_DOSES);
